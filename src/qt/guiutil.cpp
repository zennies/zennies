...
{
    // ZeniCoin: check prefix
    if(uri.scheme() != QString("ZeniCoin"))
        return false;
...
{
    // Convert ZeniCoin:// to ZeniCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ZeniCoin://"))
    {
        uri.replace(0, 12, "ZeniCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ZeniCoin.lnk";
}
...
{
    return GetAutostartDir() / "ZeniCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ZeniCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ZeniCoin-qt [" + tr("command-line options") + "]                     " + "\n";
